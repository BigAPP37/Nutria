-- Alinea user_tdee_state con lo que consume la app web.
-- La tabla existente solo guardaba TDEE adaptativo, pero la UI y el onboarding
-- también necesitan objetivo calórico y macros.

ALTER TABLE public.user_tdee_state
  ADD COLUMN IF NOT EXISTS goal_kcal INTEGER,
...
BEGIN
  ...
  VALUES (
    ...
    p_goal::public.user_goal,
    p_activity_level::public.activity_level,
    FALSE,
    COALESCE(NULLIF(upper(p_country_code), ''), 'ES'),
    p_unit_weight::public.unit_weight,
    p_unit_energy::public.unit_energy,
    p_timezone,
    v_now
  )
  ...
  VALUES (
    v_user_id,
    ROUND(p_tdee)::INTEGER,
    ROUND(p_tdee)::INTEGER,
    p_goal_kcal,
    ROUND(p_protein_g)::INTEGER,
    ROUND(p_carbs_g)::INTEGER,
    ROUND(p_fat_g)::INTEGER,
    v_now
  )
  ON CONFLICT (user_id) DO UPDATE
  SET
    current_tdee_kcal = EXCLUDED.current_tdee_kcal,
    initial_tdee_kcal = COALESCE(public.user_tdee_state.initial_tdee_kcal, EXCLUDED.initial_tdee_kcal),
