...
interface WeightEntryRow { weight_kg: number | string; recorded_at: string; }
...
export function useWeightHistory() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<WeightPoint[]> => {
      ...
      return ((data ?? []) as WeightEntryRow[]).map((w) => ({ date: w.recorded_at.split("T")[0], weight_kg: Number(w.weight_kg) }));
    },
